...
import { getSiblingConversationRecord, editLastRoundRating, checkIfRoundExists } from "@/data/conversation";
...
export async function POST(request: NextRequest) {
    ...
    const siblingRecordId = await getSiblingConversationRecord(conversationRecordId);

    if (!siblingRecordId) {
        return NextResponse.json({ error: "Conversation record not found" }, { status: 404 });
    }
