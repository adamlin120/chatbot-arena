...
import { getModelByConversationRecordId, messagesToConversationRound, findForkIndexOfTwoList } from "@/data/conversation";
...
export async function POST(request: NextRequest) {
    ...
    const prevConversationRecordRounds = conversationRecord?.prevConversationRecord?.rounds;
    ...
    const forkIndex = await findForkIndexOfTwoList(prevConversationRecordRounds, currentConversationRecordRounds);
    ...
    const forkedConversationRound = currentConversationRecordRounds.slice(forkIndex);
    ...
    if (!modelName) {
        return NextResponse.json({ error: "Invalid conversation record" }, { status: 400 });
    }
