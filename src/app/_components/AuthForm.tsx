...
function AuthForm(props: Props) {
  ...
  const invalidSignUp =
    !isPasswordValid || !isUsernameValid || !isPasswordConfirm;
    ...
    <div className="min-w-[300px] bg-white rounded-lg shadow-lg p-6">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {isSignUp && !isPasswordConfirm && confirmPassword.length > 0 && (
          <p className="text-red-500 text-sm">密碼不相符!</p>
        )}
        ...
        {props.error == "CredentialsSignin" && !isSignUp && (
          <p className="bg-red-100 text-red-600 text-center p-2">
            帳密錯誤或已被註冊，請重試！
          </p>
        )}
        {props.error == "NotVerified" && !isSignUp && (
          <p className="bg-green-100 text-green-600 text-center p-2">
            Email驗證信已寄出！
          </p>
        )}
      </form>
